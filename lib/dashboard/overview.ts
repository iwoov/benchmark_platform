...
export type PlatformAdminProjectOption = {
    id: string;
    name: string;
    code: string;
...
export type PlatformAdminProjectFamilyOption = {
    value: string;
    label: string;
    projectCount: number;
};
...
function emptyPlatformAdminOverview(
    selectedProjectId: string | null = null,
    selectedProjectFamily: string | null = null,
): PlatformAdminOverviewData {
    return {
        role: "PLATFORM_ADMIN",
        scale: {
...
export async function getAdminOverview(
    ...
    options: { projectId?: string | null; projectFamily?: string | null } = {},
): Promise<AdminOverviewData> {
    const projectId = options.projectId ?? null;
    const projectFamily = projectId ? null : (options.projectFamily ?? null);

    if (!process.env.DATABASE_URL) {
        return role === "SUPER_ADMIN"
            ? emptySuperAdminOverview()
            : emptyPlatformAdminOverview(projectId, projectFamily);
    }

    try {
        return role === "SUPER_ADMIN"
            ? await getSuperAdminOverview()
            : await getPlatformAdminOverview(projectId, projectFamily);
    } catch (error) {
...
async function getPlatformAdminOverview(
    ...
    projectFamilyFilter: string | null,
): Promise<PlatformAdminOverviewData> {
    const windowStart = getWindowStart();
    const reviewProjectFilter = projectIdFilter
        ? Prisma.sql`AND "projectId" = ${projectIdFilter}`
        : projectFamilyFilter
          ? Prisma.sql`AND "projectId" IN (SELECT id FROM "Project" WHERE "projectFamily" = ${projectFamilyFilter})`
          : Prisma.empty;
    const questionProjectFilter = projectIdFilter
        ? Prisma.sql`AND q."projectId" = ${projectIdFilter}`
        : projectFamilyFilter
          ? Prisma.sql`AND q."projectId" IN (SELECT id FROM "Project" WHERE "projectFamily" = ${projectFamilyFilter})`
          : Prisma.empty;
    const subjectStatsQuery = prisma.$queryRaw<
        Array<{
            primary_value: string | null;
            total: bigint;
            manual_reviewed: bigint;
            manual_approved: bigint;
            manual_rejected: bigint;
        }>
    >`
        WITH latest_manual AS (
            SELECT DISTINCT ON ("projectId", "datasourceId", "externalRecordId")
                "projectId", "datasourceId", "externalRecordId", decision
            FROM "Review"
            WHERE (
                    suggestions IS NULL
                    OR (
                        COALESCE(suggestions->>'source', '') != 'AI_STRATEGY'
                        AND COALESCE(suggestions->>'autoGenerated', 'false') != 'true'
                    )
                )
                ${reviewProjectFilter}
        ...
        SELECT
            ...
            COUNT(*) FILTER (WHERE lm.decision = 'REJECT'::"ReviewDecision")::bigint AS manual_rejected
        FROM "Question" q
        LEFT JOIN latest_manual lm
            ON lm."projectId" = q."projectId"
            AND lm."datasourceId" = q."datasourceId"
            AND lm."externalRecordId" = q."externalRecordId"
        WHERE q."isLatestRevision" = true
            ${questionProjectFilter}
    ...
    const projectFamilyCounts = new Map<string, number>();

    for (const project of projectOptions) {
        const projectFamily = project.projectFamily?.trim();

        if (projectFamily) {
            projectFamilyCounts.set(
                projectFamily,
                (projectFamilyCounts.get(projectFamily) ?? 0) + 1,
            );
        }
    }

    const projectFamilies = [...projectFamilyCounts.entries()]
    ...
    return {
        ...
        selectedProjectFamily: projectFamilyFilter,
    };
}
