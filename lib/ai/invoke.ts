...
function buildAnthropicPayload(
  ...
  const resolvedMaxTokens = input.maxTokens ?? config.maxTokensDefault ?? 2048;
...
function buildHeaders(
  ...
  if (protocol === "GEMINI_COMPATIBLE" && route.providerCode === "modelrouter") {
    return {
...
function summarizeAttempts(attempts: AttemptError[]) {
  return attempts
    .map(
      (attempt) =>
        `${attempt.providerCode}/${attempt.endpointCode}: ${attempt.error}`,
    )
    .join(" | ");
}
...
export async function invokeAiModel(
  ...
  const maxAttempts = config.allowFallback
    ? Math.min(routes.length, config.maxRetries + 1)
    : Math.min(routes.length, 1);
  const candidateRoutes = routes.slice(0, maxAttempts);
  ...
  return {
    ...
    stream,
    attempts,
    error: `模型 ${input.modelCode} 调用失败，已尝试全部路由：${summarizeAttempts(
      attempts,
    )}`,
  };
}
